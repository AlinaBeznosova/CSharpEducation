...
public class Phonebook
{
  ...
  public Subscriber GetSubscriber(Guid id)
  {
    ...
    if((this.subscribers.Single(s => s.Id == id))== null)
  ...
  public void AddNumberToSubscriber(Subscriber subscriber, PhoneNumber number)
  {
    ...
    {
      throw new ArgumentException("Нельзя добавить пустой номер.");
    }
    if (subscriber.PhoneNumbers.Any(n => n.Number == number.Number))
    {
      throw new ArgumentException("Такой номер уже существует.");
    }
  ...
  public void RenameSubscriber(Subscriber subscriber, string newName)
  {
    ...
    {
      throw new ArgumentException("Абонент с таким идентификатором не существует.");
    }
    if (string.IsNullOrEmpty(newName))
    {
      throw new ArgumentException("Нельзя изменить имя на пустое.");
    }
    if(subscriber.Name == newName)
    {
      throw new ArgumentException("Нельзя изменить имя на такое же.");
    }
  ...
  {
    if (!subscribers.Any(s => s.Id == subscriberToDelete.Id))
    {
      throw new ArgumentException("Абонент с таким идентификатором не существует.");
    }
